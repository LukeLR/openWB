...
class KostalSemCounter:
    ...
    def update(self):
        with self.__tcp_client:
            ...
            frequency = self.__tcp_client.read_holding_registers(26, ModbusDataType.UINT_32, slave=71) * 0.001

            powers = []
            for reg in [40, 80, 120]:
                powers_temp = self.__tcp_client.read_holding_registers(reg, [ModbusDataType.UINT_32]*2, slave=71)
                powers.append((powers_temp[0] if powers_temp[0] >= powers_temp[1] else -powers_temp[1]) * 0.1)

            power_temp = self.__tcp_client.read_holding_registers(0, [ModbusDataType.UINT_32]*2, slave=71)
